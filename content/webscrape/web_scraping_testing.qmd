...
```{r}
purrr::walk(sample_cbpp$url, browseURL)
```
...
sample_cbpp <- sample_cbpp |>
  mutate(css_text = case_when(
    ...
    url_type == "press" ~ '[class="block block-layout-builder block-field-blocknoderich-contentbody"]',
...
```{r}
test <- article_pull_df(sample_cbpp[21])
```
...
handlers("cli")
test_foreach <- text_coll_foreach(sample_cbpp)
test_foreach_js <-text_coll_foreach_js(sample_cato, remote_driver)
```
