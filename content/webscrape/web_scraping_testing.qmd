...
```{r}
cap_sample$css_text <- '[class="wysiwyg -xw:4 -mx:a"]'
cap_sample$css_title <- ".header2-title"
cap_sample$css_date <- '[class="-t:9 -tt:u -c:d2t"]'
cap_sample$css_author <- '[class="authors1-list -as:2 -t:10"]'
cap_sample$css_topics <- '[class="-c:a5t term_link_listing"]'
```

# Opening Sample Links in Browser

```{r}
purrr::walk(cato_sample$url, browseURL)
```
...
```{r}
cato_sample <- cato_sample |>
  mutate(css_text = case_when(
    url_type == "report" ~ ".article-title",
    url_type == "blog" ~ ".body-text"
  ),
  css_title = case_when(
    url_type == "report" ~ ".article-title",
    url_type == "blog" ~ ".h2"
  ...
  css_date = case_when(
    url_type == "report" ~ '[property="article:published_time"]',
    url_type == "blog" ~ ".date-time__date"
  ),
  css_author = case_when(
    url_type == "report" ~ ".mb-2",
    url_type == "blog" ~ "a.me-4"
  ),
  css_topics = case_when(
    url_type == "report" ~ NA,
    url_type == "blog" ~ ".block--inner"
  ))
```

```{r}

test <- article_pull_df(cato_sample[11,])
```
...
handlers("cli")
foreach_test <- text_coll_foreach(cap_sample)
```
