...
here::i_am("content/modeling/bigram_nlp_modeling_xgb_bow.qmd")
library(tidymodels)
...
readr::write_rds(tune_rs, "model_data/tune_results_xgb_bow.rds")
```
...
tidytable::fwrite(xgb_rs_metrics, here::here("content", "modeling", "model_results", "xgb_rs_metrics_bow_bigrams_4k.csv"))
...
ggsave("plots/cv_metrics_xgb_bow.png")
```
...
ggsave("plots/variable_importance_plot_xgb_bow.png")
```

```{r}
last_fit_xgb <- last_fit(final_xgb, bow_split)

xgb_metric <- last_fit_xgb |>
  collect_predictions() |>
  class_metrics(source_bias, estimate = .pred_class, `.pred_left-wing`)

tidytable::fwrite(xgb_metric, "model_results/xgb_test_metrics_bow.csv")
...
ggsave("plots/roc_curve_xgb_bow.png")
...
ggsave("plots/confusion_matrix_xgb_bow.png")
```
