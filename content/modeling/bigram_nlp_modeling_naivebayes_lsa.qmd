...
readr::write_rds(tune_rs, "model_results/tune_results_naivebayes_lsa.rds", compress = "gz")
```
...
```{r}
last_fit_naivebayes <- last_fit(final_naivebayes, lsa_split)

naivebayes_metric <- last_fit_naivebayes |>
  collect_predictions() |>
  class_metrics(source_bias, estimate = .pred_class, `.pred_left-wing`)

tidytable::fwrite(naivebayes_metric, "model_results/naivebayes_test_metrics_lsa.csv")
...
ggsave("plots/confusion_matrix_naivebayes_lsa.png")
```
