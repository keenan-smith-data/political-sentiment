...
- LSA utilizes Truncated Singular Value Decomposition
  ...
  - $A_{mxn} = U_{mxr}S_{rxr}V_{nxr}^T$ [@svd_math]
    - $m$ are the documents, $n$ are the features, $r$ are the components
  - Mathematically Similar to Principal Components Analysis
  - More flexible and can use Sparse Matrices
- LSA groups mathematically similar features together semantically 
- 200 *components* were chosen for Ease of Computation
  - Some information is lost, but some insight is gained into semantically similar combinations
