...
# filtering <- topfeatures(bigram_trim, n = ) |> as.list() |> as_tidytable() |> pivot_longer()

# specifics <- filtering |> filter(stringr::str_detect(name, "^pdf_"))
...
matrix_to_tidy <- function(mat) {
  ...
  colnames(df) <- c("docid", paste0("component", 1:400))
  return(df)
}

bigrams_lsa_docs_df_400 <- matrix_to_tidy(bigrams_lsa_docs_400)

bigrams_lsa_features_df_400 <- matrix_to_tidy(bigrams_lsa_features_400)
...
```{r}
bigrams_lsa <- textmodel_lsa(dfmat_bigrams_tfidf_lsa, nd = 200)

bigrams_lsa_docs <- bigrams_lsa$docs
bigrams_lsa_features <- bigrams_lsa$features
bigrams_lsa_sk <- bigrams_lsa$sk

matrix_to_tidy <- function(mat) {
  df <- tidytable::as_tidytable(mat, .keep_rownames = "docid")
  colnames(df) <- c("docid", paste0("component", 1:200))
  return(df)
}

bigrams_lsa_docs_df <- matrix_to_tidy(bigrams_lsa_docs)

bigrams_lsa_features_df <- matrix_to_tidy(bigrams_lsa_features)

d_mat <- diag(bigrams_lsa_sk)

fwrite(bigrams_lsa_docs_df, here::here("data", "model_data", "lsa_bigram_docs.csv.gz"))
fwrite(bigrams_lsa_features_df, here::here("data", "model_data", "lsa_bigram_features.csv.gz"))
fwrite(d_mat, here::here("data", "model_data", "lsa_bigram_D.csv.gz"))
