...
bigram_trim <- dfm_remove(dfmat_polsent_bigrams, pattern = bigram_stopterms_post_write) |>
  dfm_select(pattern = c("^author_+", "^article_+", "\\w+_author$", "\\w+_article"), selection = "remove", valuetype = "regex")

bigram_trim <- dfm_select(bigram_trim, pattern = c("^que_+"), selection = "remove", valuetype = "regex")

bigram_trim <- dfm_keep(bigram_trim, min_nchar = 6)
...
specifics <- filtering |>
  filter(stringr::str_detect(name, "que"))
