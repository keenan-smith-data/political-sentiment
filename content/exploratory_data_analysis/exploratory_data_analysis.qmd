...
source_tibble <- dplyr::collect(source_table)
...
dfmat_bigrams_trim <- dfm_trim(dfmat_polsent_bigrams, min_termfreq = 3000, termfreq_type = "rank")
...
```{r}
convert_word <- convert(dfmat_words_trim, to = "data.frame")
fwrite(convert_word, here::here("content", "project", "word_df.csv.gz"))

convert_bigrams <- convert(dfmat_bigrams_trim, to = "data.frame")
fwrite(convert_bigrams, here::here("content", "project", "bigram_df.csv.gz"))

convert_word_tfidf <- convert(dfmat_words_tfidf, to = "data.frame")
fwrite(convert_word_tfidf, here::here("content", "project", "tfidf_word_df.csv.gz"))

convert_bigrams_tfidf <- convert(dfmat_bigrams_tfidf, to = "data.frame")
fwrite(convert_bigrams_tfidf, here::here("content", "project", "tfidf_bigram_df.csv.gz"))
...
tstat_word_freq <- textstat_frequency(dfmat_words_trim, n = 200, groups = source_bias)
fwrite(tstat_word_freq, here::here("content", "project", "tstat_freq_word.csv"))

left_word_freq <- subset(tstat_word_freq, group == "left-wing")
right_word_freq <- subset(tstat_word_freq, group == "right-wing")
```
...
wordcloud2::wordcloud2(left_word_freq, color = pals::ocean.thermal(100), minRotation = pi/6, maxRotation = pi/6, rotateRatio = 1)
wordcloud2::wordcloud2(right_word_freq, color = pals::ocean.oxy(100), minRotation = -pi/6, maxRotation = -pi/6, rotateRatio = 1)
...
textplot_wordcloud(dfmat_group, comparison = TRUE, max_words = 200, color = c("red", "blue"))
```
