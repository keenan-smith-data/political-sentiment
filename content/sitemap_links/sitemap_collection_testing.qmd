...
sitemap_function <- function(sitemap,
                             sitemap_index = TRUE,
                             filter_col = "loc",
                             skip = FALSE)
{
...
sitemap_to_format <- function(loc_links,
  ...
  temp <- temp |>
    ...
    tidytable::mutate(loc = as.character(loc),
                      lastmod = lubridate::date("1970-01-01"),
...
hoover_sitemap_links <- sitemap_function(hoover_sitemap, sitemap_index = FALSE)
# Sitemaps that Need to skip the first Row
claremont_sitemap_links <- sitemap_function(claremont_sitemap, sitemap_index = FALSE, skip = TRUE)
discovery_sitemap_links <- sitemap_function(discovery_sitemap, sitemap_index = FALSE, skip = TRUE)
```
...
links_cbpp <- links_cbpp |> mutate(art_souce = "Center on Budget and Policy Priorities",
                                   art_bias = "left-wing")
