...
CREATE TABLE full_corpus_filtered(art_link VARCHAR PRIMARY KEY, art_date DATE, art_author VARCHAR, art_title VARCHAR, art_source VARCHAR, full_text VARCHAR, pull_index INTEGER, short_source VARCHAR, source_bias VARCHAR, length_text INTEGER);
...
CREATE UNIQUE INDEX text_hrw_idx ON 'text_hrw' (art_link, art_source);
